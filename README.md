...
- Git が実際に実行する hook の置き場所は `.git/hooks/` です
- ただし `.git/` はローカル管理領域で、通常は Git 管理の対象にもならず、VS Code からも見えにくいことがあります
- そのため、このリポジトリでは共有用の元ファイルを [hooks/pre-push](hooks/pre-push) に置き、[setup.ps1](setup.ps1) / [setup.sh](setup.sh) で `.git/hooks/pre-push` へコピーする構成にしています
- GitHub の `.github/` 配下は GitHub Actions や template 用で、local hook の実行場所ではありません

関連ファイル:

- [hooks/README.md](hooks/README.md): `hooks/` ディレクトリの役割説明
- [setup.ps1](setup.ps1): PowerShell で `.git/hooks` へインストールする
- [setup.sh](setup.sh): bash で `.git/hooks` へインストールする
...
![Architecture Diagram Japanese](outputs/pre-push-security-gate-architecture-ja.drawio.svg)

画像ファイル:

- [outputs/pre-push-security-gate-architecture.drawio.svg](outputs/pre-push-security-gate-architecture.drawio.svg)
- [outputs/pre-push-security-gate-architecture-ja.drawio.svg](outputs/pre-push-security-gate-architecture-ja.drawio.svg)

編集元 drawio:

- [outputs/pre-push-security-gate-architecture.drawio](outputs/pre-push-security-gate-architecture.drawio)
- [outputs/pre-push-security-gate-architecture-ja.drawio](outputs/pre-push-security-gate-architecture-ja.drawio)

見どころ:

- Git の `pre-push` フックは bash ラッパーだが、実際のレビューエンジンは PowerShell 7 が正本
- commit 差分生成と regex ベースの静的シグナル収集はローカルで完結する
- AI 判定はローカルで実行される Copilot CLI に集約し、最終的な allow / block を hook が Git に返す
...
- VS Code のエクスプローラーでは `.git/` 配下を普段確認しづらいので、編集対象は原則として [hooks/pre-push](hooks/pre-push) 側です
- `.git/hooks/pre-push` はインストール先の実体で、必要なら [setup.ps1](setup.ps1) か [setup.sh](setup.sh) を再実行して同期します
